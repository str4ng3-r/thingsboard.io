...
:::tip
In v4.0, Astro added built-in support for i18n routing that allows you to configure default and supported languages and includes valuable helper functions to assist you in serving an international audience. If you want to use this instead, see our [internationalization guide](/guides/internationalization/) to learn about these features.
:::
This example serves each language at its own subpath, e.g. `example.com/blog` for English and `example.com/fr/blog` for French.

If you prefer the default language to not be visible in the URL unlike other languages, there are [instructions to hide the default language](/recipes/i18n/#hide-default-language-in-the-url) below.
...
2. Set up `src/pages/index.astro` to redirect to your default language.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        If you are using an SSR adapter, you can use [`Astro.redirect`](/guides/routing/#dynamic-redirects) to redirect to the default language on the server.
...
2. Create a `src/content.config.ts` file and export a collection for each type of content.
    ...
    <ReadMore>Read more about [Content Collections](/guides/content-collections/).</ReadMore>

3. Use [dynamic routes](/guides/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        In [SSR mode](/guides/on-demand-rendering/), fetch the requested entry directly:
    ...
    <ReadMore>Read more about [dynamic routing](/guides/routing/#dynamic-routes).</ReadMore>
...
4. Each page must have a `lang` attribute on the `<html>` element that matches the language on the page. In this example, a [reusable layout](/basics/layouts/) extracts the language from the current route:
