...
Astro uses a [middleware](/guides/middleware/) to implement its routing logic. This middleware function is placed in the [first position](/guides/middleware/#chaining-middleware) where it awaits every `Response` coming from any additional middleware and each page route before finally executing its own logic.
...
Both a list of all supported languages ([`locales`](/reference/configuration-reference/#i18nlocales)) and a default language ([`defaultLocale`](/reference/configuration-reference/#i18ndefaultlocale)), which must be one of the languages listed in `locales`, need to be specified in an `i18n` configuration object. Additionally, you can configure more specific routing and fallback behavior to match your desired URLs.
...
Organize your content folders with localized content by language. Create individual `/[locale]/` folders anywhere within `src/pages/` and Astro's [file-based routing](/guides/routing/) will create your pages at corresponding URL paths.

Your folder names must match the items in `locales` exactly. Include a localized folder for your `defaultLocale` only if you configure `prefixDefaultLocale: true` to show a localized URL path for your default language (e.g. `/about/`).
...
This routing option defines whether or not your default language's URLs should use a language prefix (e.g. `/about/`).
...
This setting also determines where the page files for your default language must exist (e.g. `src/pages/about/` or `src/pages/about`) as the file structure and URL structure must match for all languages.
...
You can opt out of this behavior by [setting `redirectToDefaultLocale: false`](/reference/configuration-reference/#i18nroutingredirecttodefaultlocale). This allows you to have a site home page that exists outside of your configured locale folder structure.
...
You can run `middleware` with [routing options](#routing) in combination with your own middleware, using the [`sequence`](/reference/modules/astro-middleware/#sequence) utility to determine the order:
...
This routing option allows you to customize your domains on a per-language basis for `server` rendered projects using the [`@astrojs/node`](/guides/integrations-guide/node/) or [`@astrojs/vercel`](/guides/integrations-guide/vercel/) adapter with a `site` configured.
...
Your fallback strategy consists of two parts: choosing which languages should fallback to which other languages ([`i18n.fallback`](/reference/configuration-reference/#i18nfallback)) and choosing whether to perform a [redirect](/guides/routing/#redirects) or a [rewrite](/guides/routing/#rewrites) to show the fallback content ([`i18n.routing.fallbackType`](/reference/configuration-reference/#i18nroutingfallbacktype) added in Astro v4.15.0).
...
When using functions from the [`astro:i18n` virtual module](/reference/modules/astro-i18n/) to compute valid URL paths based on your configuration (e.g. `getRelativeLocaleUrl()`), [use the `path` as the value for `locale`](/reference/modules/astro-i18n/#getlocalebypath).
...
- [`Astro.preferredLocale`](/reference/api-reference/#preferredlocale):  Astro can compute a **preferred locale** for your visitor if their browser's preferred locale is included in your `locales` array. This value is undefined if no such match exists.

- [`Astro.preferredLocaleList`](/reference/api-reference/#preferredlocalelist): An array of all locales that are both requested by the browser and supported by your website. This produces a list of all compatible languages between your site and your visitor. The value is `[]` if none of the browser's requested languages are found in your `locales` array. If the browser does not specify any preferred languages, then this value will be [`i18n.locales`].

- [`Astro.currentLocale`](/reference/api-reference/#currentlocale): The locale computed from the current URL, using the syntax specified in your `locales` configuration. If the URL does not contain a `/[locale]/` prefix, then the value will default to [`i18n.defaultLocale`](/reference/configuration-reference/#i18ndefaultlocale).
...
[`site`]: /reference/configuration-reference/#site
[`i18n.locales`]: /reference/configuration-reference/#i18nlocales
