...
Astro's [React integration](/guides/integrations-guide/react/) provides support for [using React components inside Astro components](/guides/framework-components/), including entire React apps like Create React App (CRA)!
...
- [content-driven](/concepts/why-astro/#content-driven): Astro was designed to showcase your content and to allow you to opt-in to interactivity only as needed. An existing CRA app might be built for high client-side interactivity and may require advanced Astro techniques to include items that are more challenging to replicate using `.astro` components, such as dashboards.
...
Your app needs a [client directive](/reference/directives-reference/#client-directives) for interactivity. Astro will render your React app as static HTML until you opt-in to client-side JavaScript.
...
You will replicate a similar component-based design [using Astro HTML templating components for your basic structure](/basics/astro-components/) while importing and including individual React components (which may themselves be entire apps!) for islands of interactivity.
...
You may find it helpful to learn about [Astro islands](/concepts/islands/) and [Astro components](/basics/astro-components/) before restructuring your CRA as an Astro project.
...
4. Use [`Astro.props`](/reference/api-reference/#props) to access any additional props that were previously passed to your CRA function.
...
6. Replace `useEffect()` with import statements or [`import.meta.glob()`](/guides/imports/#importmetaglob) to query your local files. Use `fetch()` to fetch external data.
...
To [fetch remote data](/guides/data-fetching/), use `fetch()`.
...
If you found (or made!) a helpful video or blog post about converting a Create React App to Astro, [add it to this list](https://github.com/withastro/docs/edit/main/src/content/docs/guides/migrate-to-astro/from-create-react-app.mdx)!
:::
