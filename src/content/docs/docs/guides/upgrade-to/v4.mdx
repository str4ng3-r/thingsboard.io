...
Need to upgrade an older project to v3? See our [older migration guide](/guides/upgrade-to/v3/).

Need to see the v3 docs? Visit this [older version of the docs site (unmaintained v3.6 snapshot)](https://web.archive.org/web/20231203051122/https://docs.astro.build/getting-started/).
...
If you need to refer to the documentation for a v3.x project, you can browse this [(unmaintained) snapshot of the docs from before v4.0 was released](https://web.archive.org/web/20231203051122/https://docs.astro.build/getting-started/).
...
In Astro v3.0, `build.split` and `build.excludeMiddleware` build config options were deprecated and replaced with [adapter configuration options](/reference/adapter-reference/#adapter-features) to perform the same tasks.
...
Please see the v3 migration guide to [update these deprecated middleware properties](/guides/upgrade-to/v3/#deprecated-buildexcludemiddleware-and-buildsplit) with adapter configurations.
...
Update all occurrences to [`Astro.params`](/reference/api-reference/#params), which is the supported replacement.
...
If you are using the unsupported method for generating RSS feeds, you must now use the [`@astrojs/rss` integration](/recipes/rss/) for a complete RSS setup.
...
Please see the v3 migration guide [for guidance using uppercase HTTP request methods](/guides/upgrade-to/v3/#changed-http-request-methods-case).
...
When using the Astro preview server, all of your static asset imports and URLs from the public directory must have [the base value](/reference/configuration-reference/#base) prefixed to the path.
...
Know a good resource for Astro v4.0? [Edit this page](https://github.com/withastro/docs/edit/main/src/content/docs/guides/upgrade-to/v4.mdx) and add a link below!
