...
<ReadMore>Read more about [using environment variables](/guides/environment-variables/) and `.env` files in Astro.</ReadMore>
...
This example uses Astro's default static mode, and creates [a dynamic routing page file](/guides/routing/#dynamic-routes) with the `getStaticPaths()` function. This function will be called at build time to generate the list of paths that become pages.
...
If you found (or made!) a helpful video or blog post about using Drupal with Astro, [add it to this list](https://github.com/withastro/docs/edit/main/src/content/docs/guides/cms/drupal.mdx)!
:::
