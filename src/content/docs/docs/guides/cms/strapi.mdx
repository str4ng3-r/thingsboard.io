...
<ReadMore>Learn more about [environment variables](/guides/environment-variables/) and `.env` files in Astro.</ReadMore>
...
Create the file `src/pages/blog/[slug].astro` to [dynamically generate a page](/guides/routing/#dynamic-routes) for each article.
...
In Astro's default static mode (SSG), use [`getStaticPaths()`](/reference/routing-reference/#getstaticpaths) to fetch your list of articles from Strapi.
...
```
:::tip
Make sure to choose the right rendering for your content. For markdown check out our [markdown guide](/guides/markdown-content/). If you are rendering html refer to [this guide](/reference/directives-reference/#sethtml) for safety.
:::

#### On-demand rendering

If you've [opted into on-demand rendering with an adapter](/guides/on-demand-rendering/), [generate your dynamic routes](/guides/routing/#on-demand-dynamic-routes) using the following code:
...
To deploy your website, visit our [deployment guides](/guides/deploy/) and follow the instructions for your preferred hosting provider.
