...
See our separate guide for [deploying to Firebase hosting](/guides/deploy/firebase/).
...
- An Astro project with [`output: 'server'` for on-demand rendering](/guides/on-demand-rendering/) enabled.
...
:::tip
Read more about [environment variables](/guides/environment-variables/) and `.env` files in Astro.
:::
...
Firebase authentication in Astro requires the following three [Astro server endpoints](/guides/endpoints/):
