...
You can add the [`is:inline`](/reference/directives-reference/#isinline) directive to intentionally opt out of processing for a script.
...
In Astro components, the code in [the frontmatter](/basics/astro-components/#the-component-script) (between the `---` fences) runs on the server and is not available in the browser.
...
Elements rendered by a UI framework may not be available yet when a `<script>` tag executes. If your script also needs to handle [UI framework components](/guides/framework-components/), using a custom element is recommended.
