...
You don't need to write TypeScript code in your Astro projects to benefit from it. Astro always treats your component code as TypeScript, and the [Astro VS Code Extension](/editor-setup/) will infer as much as it can to provide autocompletion, hints, and errors in your editor.
...
This plugin runs only in the editor. When running `tsc` in the terminal, `.astro` files are ignored entirely. Instead, you can use [the `astro check` CLI command](/reference/cli-reference/#astro-check) to check both `.astro` and `.ts` files.
...
Astro supports import aliases that you define in your `tsconfig.json` `paths` configuration. [Read our imports guide](/guides/imports/#aliases) to learn more.
...
Astro includes helpers for working with the types returned by your [`getStaticPaths()`](/reference/routing-reference/#getstaticpaths) function for dynamic routes.

You can get the type of [`Astro.params`](/reference/api-reference/#params) with `InferGetStaticParamsType` and the type of [`Astro.props`](/reference/api-reference/#props) with `InferGetStaticPropsType` or you can use `GetStaticPaths` to infer both at once:
...
<ReadMore>Read more about [`.ts` file imports](/guides/imports/#typescript) in Astro.</ReadMore>
